...
# Minimize PowerShell window, but isn't the best experience as it may not get focus
# $action = New-ScheduledTaskAction -Execute "PowerShell.exe" -Argument "-WindowStyle Hidden -NoProfile -ExecutionPolicy Bypass -EncodedCommand $encodedCommand"

# Run using `conhost --headless` to that a PowerShell window doesn't appear, but it will be in the background
# $action = New-ScheduledTaskAction -Execute "C:\Windows\System32\conhost.exe" -Argument "--headless powershell.exe -NoProfile -ExecutionPolicy Bypass -EncodedCommand $encodedCommand"
